export async function download(idShip,idchuyenBien,action, error){
  try {
        const response = await fetch(`http://localhost:3000/api/download?id=${idShip}&&id_cb=${idchuyenBien}`);
...
export async function DanhSachTau(statuss,action,err){
   try {
    ...
    const response = await fetch(`http://localhost:3000/api/ship`);
...
export async function DanhSachChuyenBien(idShip,action,err){
   try {
        ...
        const response = await fetch(`http://localhost:3000/api/chuyen-bien?id=${idShip}`);
...
export async function TimTauTheoId(query, action, err) {
    try {
        ...
        if (!Array.isArray(data)) {
            const resp = await fetch(`http://localhost:3000/api/ship`);
...
export async function Locations(idChuyenBien,action,err){
   try {
        // 1. Thực hiện Fetch và chờ Response
        const response = await fetch(`http://localhost:3000/api/locations?id=${idChuyenBien}`);
